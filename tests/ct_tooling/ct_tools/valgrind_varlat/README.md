...
## Vlagrind Install Requirements
In order to successfully execute valgrind_varlat's test using the tooling developed in this subrepository follow the next steps:
...
## Algorithm Testing
Because of how many warnings are output, it is not feasible to store all the warnings in terms of memory and runtime. Therefore, the `on_liboqs.sh` script handles valgrind_varlat's output as follows:
