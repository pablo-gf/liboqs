document.getElementById('filter-select').addEventListener('change', function() {
  ...
  main.innerHTML = ''; // Clear existing graphs
  ...
  const filteredData = window.BENCHMARK_DATA.entries.filter(entry => {
    if (filter === 'all') return true;
    if (filter === 'kem') return kemAlgorithms.includes(entry.name);
    if (filter === 'signature') return signatureAlgorithms.includes(entry.name);
    return false;
  });

  filteredData.forEach(entry => {
    const dataSet = collectBenchesPerTestCase(entry.data);
    renderBenchSet(entry.name, dataSet, main);
  });
});
